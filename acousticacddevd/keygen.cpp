...
void process_serial(TCHAR *name, TCHAR *serial);
...
DWORD WINAPI GenerateSerial(HWND hwnd){
	...
	int selection = 0;
	if(GetDlgItemText(hwnd,IDC_NAME,(TCHAR*)name,MAX_NAME)<MIN_NAME)
	{
		SetDlgItemText(hwnd, IDC_SERIAL,L"Please enter a longer name...");
		return 1;
	}
	srand(GetTickCount());
	memset(serial,0,MAX_SERIAL);
	process_serial((TCHAR*)name, (TCHAR*)serial);
	...
	case WM_COMMAND:
		switch (LOWORD(wParam)) {
			...
			case IDC_EXIT:
				...
				break;
			case IDC_ABOUT:
				MessageBox(hwndDlg,about,L"About this keygen....",MB_OK|MB_ICONEXCLAMATION);
		}
		break;
	case WM_CLOSE:
		EndDialog(hwndDlg,0);
