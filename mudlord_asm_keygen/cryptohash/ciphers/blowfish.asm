comment " data   = 8 bytes (64-bit)         "
comment " keylen = 1..56 bytes (8..448-bit) "
comment " rounds = 16                       "
...
align dword
bf_initpbox label dword
dd 0243F6A88h,085A308D3h,013198A2Eh,003707344h,0A4093822h,0299F31D0h,0082EFA98h,0EC4E6C89h
dd 0452821E6h,038D01377h,0BE5466CFh,034E90C6Ch,0C0AC29B7h,0C97C50DDh,03F84D5B5h,0B5470917h
dd 09216D5D9h,08979FB1Bh

align dword
bf_initsbox label dword
dd 0D1310BA6h,098DFB5ACh,02FFD72DBh,0D01ADFB7h,0B8E1AFEDh,06A267E96h,0BA7C9045h,0F12C7F99h
...
.data?
bf_pbox dd 18 dup(?)
bf_sbox dd 4*256 dup(?)
bf_block dd 2 dup(?)

.code

align dword
BlowfishInit proc uses esi edi ebx pKey:DWORD,dwKeyLen:DWORD
	mov edi,offset bf_sbox
	mov esi,offset bf_initsbox
	mov ecx,(sizeof bf_sbox)/4
	rep movsd
	mov ebx,dwKeyLen
	mov esi,pKey
	.if ebx >= 4 && ebx <= 56
		...
		.repeat
			...
			.repeat
				...
				inc edx
			.until edx == 4
			xor	eax,dword ptr [bf_initpbox+4*ecx]
			mov	dword ptr [bf_pbox+4*ecx],eax
			inc	ecx
		.until ecx == 18 ;BlowfishROUND+2
		xor	ebx,ebx
		mov	dword ptr [bf_block+0*4],ebx
		mov	dword ptr [bf_block+1*4],ebx
		.repeat
			invoke BlowfishEncrypt,offset bf_block,offset bf_block
			bswap eax
			bswap edx
			mov	dword ptr [bf_pbox+2*4*ebx+0*4],eax
			mov	dword ptr [bf_pbox+2*4*ebx+1*4],edx
			inc ebx
		.until ebx == 9; ( (BlowfishROUND+2) / 2))
		xor edi,edi
		.repeat
			xor ebx,ebx
			.repeat
				...
				bswap eax
				bswap edx
				mov	dword ptr [bf_sbox+2*4*ebx+edi+0*4],eax
				mov	dword ptr [bf_sbox+2*4*ebx+edi+1*4],edx
				inc ebx
			.until ebx == 128
			add	edi,4*256
		.until edi == 4*4*256 
...
align dword
BlowfishEncrypt proc uses esi edi ebx pBlockIn:DWORD,pBlockOut:DWORD
	mov esi,pBlockIn
	xor ecx,ecx;n==0
	mov eax,[esi+0*4]
	mov edx,[esi+1*4]
	...
	bswap edx
	.repeat
		xor eax,bf_pbox[ecx*4]
		mov edi,eax
		mov esi,eax
		shr edi,24
		shr esi,16
		and esi,0FFh
		mov ebx,bf_sbox[edi*4+0*256*4]
		add ebx,bf_sbox[esi*4+1*256*4]
		movzx edi,ah
		movzx esi,al
		xor ebx,bf_sbox[edi*4+2*256*4]
		add ebx,bf_sbox[esi*4+3*256*4]
		xor ebx,edx
		inc ecx;n++
		mov edx,eax
		cmp ecx,16
		mov eax,ebx
	.until zero?;ecx == 16 ; BlowfishROUND
	xchg eax,edx
	xor edx,bf_pbox[16*4]; BlowfishROUND
	xor eax,bf_pbox[17*4]; BlowfishROUND+1
	mov esi,pBlockOut
	bswap eax
	bswap edx
	mov [esi+0*4],eax
...
align dword
BlowfishDecrypt proc uses esi edi ebx pBlockIn:DWORD,pBlockOut:DWORD
	mov esi,pBlockIn
	mov ecx,17; BlowfishROUND+1
	mov eax,[esi+0*4]
	mov edx,[esi+1*4]
	bswap eax
	bswap edx
